...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    formError.textContent = err.message?.includes("fetch")
      ? "Can't reach the prediction API. Make sure the FastAPI server is running and reachable on port 8000."
...
function buildProbList(paired, predicted) {
  ...
  sorted.forEach((cls) => {
    ...
    row.appendChild(valueWrapper); // Append the wrapper here
    ...
    requestAnimationFrame(() => {
      setTimeout(() => {
        ...
        animateCount(value, pct); // This continues to animate just the number!
      }, REDUCE_MOTION ? 0 : 150);
    });
  });
}
